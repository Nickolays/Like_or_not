...
class Like_or_not:
    ...
    :param figsize: tuple(int, int), default = (20, 11)
        Размер изображения при сохранении
    ...
    def __init__(self, like_path: str, d_like_path: str, start=0, duration=None, sample_rate=22050, figsize=(20, 11)):
    ...
    def __str__(self):
        return "Если вы уверены, что у вас все трэки имеют одинаковую частоту дискретизации (sample_rate), \ то тогда " \
               "лучше использовать библиотеку SoundFile. Эта библиотека гараздо быстрее загружает песни. В методе " \
               "_load_songs она уже есть, необходимо только поменять либросу на sf "
    ...
    def show_libraries_version(self):
        ...
        print(f'Your numpy version is {librosa.__version__}, needed: 0.9.0 and more')
        print(f'Your numpy version is {cv2.__version__}, needed: 4.5.5 and more')
        print(f'Your numpy version is {tf.__version__}, needed: 2.7.0 and more')
    ...
    def __set_photo_shape(self, path_img:str):
        ...
        get_shape = cv2.imread(os.path.join(path_img, files[0]), 0)  # 0 обозначает, что мы открываем изображение как GRAY
    ...
    def __save_fig(self, data, out_path, spec):
        ...
        # Сохраняем
        plt.axis('off')
        fig.savefig(out_path, bbox_inches='tight', dpi=145, pad_inches=-1)

        del fig, data, p

    def save_img(self, out_path=None, spec='stft'):
        """
        Функция сохраняет изображение на спектрограмме для дальнейшей обработки Свёрточной нейронной сетью.
        ...
        if out_path is None:
            ...
            elif os.path.isdir(f"Spec_pictures_{self._spectrum}"):
                pic_dir = f'Spec_pictures_{self._spectrum}_{self.__count_dirs}'
                os.mkdir(pic_dir)
                self.__count_dirs = self.__count_dirs + 1
                self._pictures_dir = pic_dir
    ...
    def _open_img_for_CNN(self, path_img=None):
        ...
        images = np.zeros(shape=(len(self._paths), self.__spec_height, self.__spec_width, 1))
    ...
    def spec_Conv2D(self, learning_rate=0.001, paht_with_imgs=None):
        ...
        model.add(keras.Input(shape=(self.__spec_height, self.__spec_width, 1)))
        # Построение модели
        if self.__spec_height > 1400 and self.__spec_width > 2800:
        ...
        #
        model.add(keras.layers.Conv2D(20, (10, 20), padding='same', activation='relu'))
        model.add(keras.layers.Conv2D(20, (5, 5), padding='same', activation='relu'))
        model.add(keras.layers.MaxPooling2D(pool_size=(5, 5), strides=2))
        #
        if self.__spec_height > 600 and self.__spec_width > 1200:
            model.add(keras.layers.Conv2D(40, (10, 10), padding='same', activation='relu'))
            model.add(keras.layers.MaxPooling2D(pool_size=(5, 10), strides=2))
        #
        model.add(keras.layers.Conv2D(70, (5, 10), padding='same', activation='relu'))
        model.add(keras.layers.MaxPooling2D(pool_size=(5, 5), strides=(2, 4)))
        #
        model.add(keras.layers.Conv2D(120, (5, 5), padding='same', activation='relu'))
        model.add(keras.layers.MaxPooling2D(pool_size=(5, 5), strides=3))
        #
        model.add(keras.layers.Conv2D(240, (3, 3), padding='same', activation='relu'))
    ...
    def spec_Conv1D(self, learning_rate=0.001):
        ...
        if 121 * self.sr < self.__min_song_shape < 60 * self.sr:
        ...
        model.add(keras.layers.MaxPool1D(pool_size=5, strides=3))  # 330 750,  220 500, 98 000
        model.add(keras.layers.Dropout(rate=0.1))
        model.add(keras.layers.Conv1D(filters=40, kernel_size=15, activation='relu', padding='same'))
        model.add(keras.layers.MaxPool1D(pool_size=5, strides=4))  # 165 375,  110 250, 24 500
        model.add(keras.layers.Dropout(rate=0.1))
        model.add(keras.layers.Conv1D(filters=50, kernel_size=10, activation='relu', padding='same'))
        model.add(keras.layers.MaxPool1D(pool_size=5, strides=5))  # 82 687, 55 125, 4 900
        model.add(keras.layers.Conv1D(filters=60, kernel_size=5, activation='relu', padding='same'))
    ...
    def spec_LSTM(self, learning_rate=0.001, paht_with_imgs=None):
        ...
        model = keras.Sequential()
        model.add(keras.Input(shape=(self.__spec_height, self.__spec_width)))
    ...
    def fit(self, epochs=100, batch_size=5, n_splits=5, device='/cpu:0', patience=5, verbose=True):
        """
    ...
    def predict(self, test_path: str, saved_model=None):
        ...
        if saved_model == None and self.__fitted_model == False:
            return print('Вы не обучили модель, пожалуйста вызовите метод fit(), перед методом predict()')
        ...
        if self._model_name in ('Conv2D', 'LSTM'):
            ...
            X_test = np.zeros(shape=(len(test_audio_path), self.__spec_height, self.__spec_width, 1))
        elif self._model_name == 'Conv1D':
        ...
        for path in tqdm.tqdm(test_audio_path, total=len(test_audio_path)):
            ...
            if self._model_name in ('Conv2D', 'LSTM'):
                ...
                if test_audio.shape[0] < self.__min_song_shape:
